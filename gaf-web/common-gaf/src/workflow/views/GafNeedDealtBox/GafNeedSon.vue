            ...
            <a-menu-item key="2" @click="hangUp">
              ...
              >
                <a-textarea
                  ...
                  placeholder=""
                />
                <button class="submit-gray" @click="hangUpOk()">确认</button>
                <button class="cancel-modal" @click="acceptCancel">取消</button>
              </a-modal>
            </a-menu-item>
            <a-menu-item key="3">
  ...
  methods: {
    ...
    async acceptSubmit() {
      ...
      if (acceptTransfer.data.isSuccessed) {
        ...
        this.$message.error(
          `获取部门用户树失败，原因：${acceptTransfer.data.message}`
        )
      }
      location.reload()
    },
    async acceptSubmitTransfer() {
      ...
      if (transferOutInfo.data.isSuccessed) {
        ...
        this.$message.error(
          `获取部门用户树失败，原因：${transferOutInfo.data.message}`
        )
      }
      location.reload()
    },
    ...
    async acceptSubmitReject() {
      ...
      if (acceptReject.data.isSuccessed) {
        ...
        this.$message.error(
          `获取部门用户树失败，原因：${acceptReject.data.message}`
        )
      }
    },
    /* 功能键 模态框 树组件遍历 */
