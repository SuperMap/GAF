...
<script>
  ...
  mounted() {
    ...
    console.log(this.editData, 'modelData')
    const copyData = { ...this.editData }
    this.dataId = copyData.fieldName
  ...
  methods: {
    ...
    submitForm() {
      this.addOrEditForm.validateFields(async (err) => {
        ...
        let url = `/data-mgt/datasource/${this.infoBox.data.datasourceId}/datasets/${this.infoBox.data.recordName}/fields/`
