...
<script>
import AddEditForm from "./FieldForm";
...
export default {
  ...
  methods: {
    ...
    async handleDelete(row) {
      const url = `/data-mgt/model-manage/fields/${row.fieldId}`;
    ...
    async getTreeData() {
      const url = `/data-mgt/model-manage/fields/type-infos?modelType=${this.mainCompent.modelData.modelType}`
      const res = await this.$axios.$get(url)
      if (res.isSuccessed) {
