<template>
  ...
  <div class="page-single">
    <gaf-table-layout v-show="!showConfigDialog && !showConfigServer">
      ...
      <template #actions>
        <button @click="showConfigServerModel" type="primary" class="btn-fun blue">
          <a-icon type="plus" />
          新增微服务配置
        </button>
        <a-popconfirm
      ...
      <template #default>
        ...
        >
          <span slot="action" slot-scope="text, record">
            ...
            <a-divider type="vertical" />
            <a @click.stop="() => deployConfig(record)"
              href="javascript:;"
              class="btn-release"
            >
              <a-icon type="arrow-up" />发布
            </a>
          </span>
        </gaf-table-with-page>
      </template>
    ...
    <div v-show="showConfigDialog">
      <config-server-update-dialog
        ...
        :refresh-load-config-server-info-list="refreshLoadConfigServerInfoList"
...
export default {
  ...
  methods: {
    ...
    rowSelect(record, selected, selectedRows) {
      ...
      this.selectedRowKeys = []
      selectedRows.forEach(item => this.selectedRowKeys.push(item.application))
    },
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (this.pagination.current !== 1 && selectedRowKeys.length === this.dataSource.length){
