...
<script>
import AddEditForm from './MenuAddOrEditForm'
  ...
  methods: {
    async onSearch(val) {
      ...
      if (val === "") {
        ...
        this.authResourceMenuList = this.searchTextApiList.filter(ltem => ltem.name.includes(val) === true)
      }
    },
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (this.pagination.current !== 1 && selectedRowKeys.length === this.authResourceMenuList.length){
    ...
    async handleDelete(row) {
      ...
      this.$nextTick(() => {
        if (this.pagination.current !== 1 &&this.authResourceMenuList.length === 1){
            this.pagination.current--
          }
        this.getList()
      })
    ...
    async getList() {
      this.loading = true
      let url = `/authority/auth-resource-menus?pageSize=${this.pagination.pageSize}&pageNum=${this.pagination.current}`
