    ...
    <gaf-table-layout>
      ...
      <template #default>
        ...
        <gaf-table-with-page
          ...
          :columns="
            columns.filter((item) => item.dataIndex !== 'resourceMenuId')
          "
          class="table-style"
          size="middle"
        >
  ...
  methods: {
    ...
    async onSearch(val) {
      ...
      if (val === "") {
        ...
        this.authResourceMenuList = this.searchTextApiList.filter(
    ...
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (
            this.pagination.current !== 1 &&
            selectedRowKeys.length === this.authResourceMenuList.length
          ) {
    ...
    async handleDelete(row) {
      ...
      this.$nextTick(() => {
        if (
          this.pagination.current !== 1 &&
          this.authResourceMenuList.length === 1
        ) {
          this.pagination.current--;
        }
        this.selectedRowKeys = this.selectedRowKeys.filter(item => {
          return item !== row.resourceMenuId
        })
        this.getList();
      });
    },
