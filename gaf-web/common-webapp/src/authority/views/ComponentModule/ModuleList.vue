      ...
      <template #default>
        ...
        >
          ...
          >
            ...
            <a-divider type="vertical" />
            <a @click.stop="() => handleRelation(record)" href="javascript:;" class="btn-code">
              <a-icon type="link" /> 关联API
            </a>
          </template>
  ...
  methods: {
    ...
    async onSearch(val) {
      ...
      if (val === "") {
        ...
        this.authResourceModuleList = this.searchTextApiList.filter(ltem => ltem.name.includes(val) === true)
      }
    },
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (this.pagination.current !== 1 && selectedRowKeys.length === this.authResourceModuleList.length){
    ...
    async handleDelete(row) {
      ...
      this.$nextTick(() => {
        if (this.pagination.current !== 1 && this.authResourceModuleList.length === 1){
            this.pagination.current--
          }
        this.getList(this.moduleGroup)
      })
