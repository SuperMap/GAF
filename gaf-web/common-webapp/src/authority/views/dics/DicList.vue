    ...
    <gaf-table-layout>
      ...
      <template #default>
        ...
        >
          ...
          >
            <a @click.stop="() => handleAddChild(record)" class="btn-code" href="javascript:;">
              <a-icon type="plus" /> 添加子项
            </a>
            <a-divider type="vertical" />
            <a @click.stop="() => handleDetail(record)"  class="btn-view" href="javascript:;">
  ...
  methods: {
    ...
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (this.pagination.current !== 1 && selectedRowKeys.length === this.sysDictList.length){
            this.pagination.current--
          }
          this.getList()
        })
    ...
    async getList() {
      ...
      if(this.searchText.trim() && this.searchedColumn && this.searchedColumn === 'dict_value') {
         url += `&dictValue=${this.searchText.trim()}`
      }
      if (this.searchText.trim() && this.searchedColumn && this.searchedColumn === 'dict_name') {
        url =
          url +
          '&searchFieldName=' +
          this.searchedColumn +
