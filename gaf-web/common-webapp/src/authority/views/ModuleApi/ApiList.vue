...
export default {
  ...
  methods: {
    async getSelected() {
      const url = `/authority/auth-module-apis/module/${this.moduleId}`
    ...
    async handleAdd() {
      ...
      //   this.$message.info('请先选中一个API')
      //   return
      // }
      const url = `/authority/auth-resource-modules/${this.moduleId}`
      const res = await this.$axios.$get(url)
      if (res.isSuccessed) {
        if (res.data.length === 0) {
          this.$message.error(`模块查找失败,原因:${res.message}`)
        } else {
          const url2 = `/authority/auth-module-apis/handle`
          const data = {
            moduleId: this.moduleId,
            apiList: this.selectedRowKeys
          }
          const res2 = await this.$axios.$post(url2, data)
          if (res2.isSuccessed) {
