        ...
        >
          ...
          <template slot="operation" slot-scope="text, record">
            ...
            <a-popconfirm
              ...
              cancel-text="取消"
            >
              <a href="javascript:;" class="btn-del"><a-icon type="stop" /> 禁用</a>
            </a-popconfirm>
            <a-popconfirm
              v-if="!record.status"
              @confirm="() => setTenantStatus(record)"
              ...
              cancel-text="取消"
            >
              <a href="javascript:;" class="btn-restart"><a-icon type="check" /> 启用</a>
            </a-popconfirm>
            <a-divider type="vertical" />
            <a
  ...
  methods: {
    ...
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (this.pagination.current !== 1 && selectedRowKeys.length === this.authTenantList.length){
    ...
    async setTenantStatus(row) {
      const url = `/authority/auth-tenants/` + row.tenantId + `?status=${!row.status}`
