    ...
    <gaf-table-layout v-show="!open">
      ...
      <template #default>
        ...
        <gaf-table-with-page
          ...
          :row-key="(r, i) => r.moduleApiId"
          :columns="columns.filter((item) => item.dataIndex !== 'moduleApiId')"
          class="table-style"
          size="middle"
        >
          <template
...
<script>
import AddEditForm from "../../views/AuthModuleApi/AddOrEditForm";
...
export default {
  ...
  data() {
    return {
      ...
      searchText: "",
      searchInput: null,
      searchedColumn: "resourceModuleId",
      sorter: {
        order: "",
        field: "",
      },
  ...
  methods: {
    ...
    async batchDel() {
      const url = "/authority/auth-module-apis/";
    ...
    async handleDelete(row) {
      const url = "/authority/auth-module-apis/" + row.moduleApiId;
    ...
    async getList() {
      ...
      this.loading = false;
      if (res.isSuccessed) {
        this.authModuleApiList = res.data.pageList;
        this.pagination.total = this.authModuleApiList.length;
      } else {
