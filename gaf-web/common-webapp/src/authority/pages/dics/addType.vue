...
export default {
  ...
  mounted() {
    ...
    this.searchTextApiList = JSON.parse(JSON.stringify(this.typeList)).map(item => {
        delete item.children
        return item
      })
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        if (rst.data.isSuccessed) {
          ...
          if(this.typeList2 && this.typeList2.length === 0) {
            this.$emit('delete-success', {catalogId:this.typeList[0].parentId,key:selectedRowKeys}, true)
          }
          this.$emit('delete-success',{catalogId:this.typeList[0].parentId,key:selectedRowKeys})
