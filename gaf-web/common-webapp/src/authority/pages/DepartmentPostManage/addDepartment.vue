...
export default {
  ...
  mounted() {
    this.departmentList2 = JSON.parse(JSON.stringify(this.departmentList)).map(
      (item) => {
        delete item.children;
        return item;
      }
    );
    this.searchTextApiList = JSON.parse(
  ...
  methods: {
    ...
    onSearch(val) {
      ...
      if (val === "") {
        ...
        this.departmentList2 = this.searchTextApiList.filter(
        ...
        if (rst.data.isSuccessed) {
          ...
          if (this.departmentList2 && this.departmentList2.length === 0) {
            this.$emit(
              "afterDeleteSuccess",
              {
                parentId: this.departmentList[0].parentId,
                key: selectedRowKeys,
              },
              true
            );
