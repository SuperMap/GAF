  ...
  <div>
    ...
    >
      <add-edit-department
        ...
        :operation="operation"
        :options="dataOfTree"
        @updateDepartmentSuccess="afterFormUpdateSuccess"
        @deleteDepartmentSuccess="afterDeleteSuccess"
...
export default {
  ...
  watch:{
    departmentList(newdata){
      this.departmentList2 = JSON.parse(JSON.stringify(newdata)).map(item => {
        delete item.children
        return item  
      })
      this.searchTextApiList = JSON.parse(JSON.stringify(newdata)).map(item => {
        delete item.children
        return item  
      })
    }
  ...
  mounted() {
    this.departmentList2 = JSON.parse(JSON.stringify(this.departmentList)).map(item => {
        delete item.children
        return item  
      })
    this.searchTextApiList = JSON.parse(JSON.stringify(this.departmentList)).map(item => {
        delete item.children
        return item  
      })
  },
  methods: {
    ...
    onSearch(val) {
      ...
      if (val === "") {
        this.departmentList2 = this.searchTextApiList
      } else {
        this.departmentList2 = this.searchTextApiList.filter(ltem => ltem.title.includes(val) === true)
      }
    },
    // 重置查询
    ...
    async handleDelete(row) {
      console.log(row)
      const url = `/authority/auth-departments/` + row.key
    ...
    // 批量删除
    async batchDel() {
      const url = '/authority/auth-departments/'
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        if (rst.data.isSuccessed) {
          this.$message.success('删除成功')
          if(this.departmentList2 && this.departmentList2.length === 0) {
            this.$emit('afterDeleteSuccess', {parentId:this.departmentList[0].parentId,key:selectedRowKeys}, true)
          }
          this.$emit('afterDeleteSuccess', {parentId:this.departmentList[0].parentId,key:selectedRowKeys})
