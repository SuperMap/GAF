  ...
  <div>
    ...
    >
      <add-edit-post
        ...
        @updatePostSuccess="afterFormUpdateSuccess"
        @deletePostSuccess="afterDeleteSuccess"
...
export default {
  ...
  watch:{
    postList(newdata){
      this.postList2 = JSON.parse(JSON.stringify(newdata)).map(item => {
  ...
  mounted() {
    this.postList2 = JSON.parse(JSON.stringify(this.postList)).map(item => {
        delete item.children
        return item  
      })
    this.searchTextApiList = JSON.parse(JSON.stringify(this.postList)).map(item => {
        delete item.children
        return item  
      })
  },
  methods: {
    ...
    async handleDelete(row) {
      ...
      console.log(row)
      const url = `/authority/auth-posts/${row.parentId}/${row.key}`
    ...
    async batchDel() {
      ...
      const url = `/authority/auth-posts/${this.postList[0].parentId}/`
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        if (rst.data.isSuccessed) {
          this.$message.success('删除成功')
          if(this.postList2 && this.postList2.length === 0) {
            this.$emit('afterDeleteSuccess', {departmentId:this.postList[0].parentId,key:selectedRowKeys}, true)
          }
          this.$emit('afterDeleteSuccess',{departmentId:this.postList[0].parentId,key:selectedRowKeys})
          this.selectedRowKeys.forEach(item => {
            this.postList.forEach((x,index) => {
              if (item === x.key){
                this.postList.splice(index,1)
              }
            })
