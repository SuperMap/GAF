...
export default {
  ...
  mounted() {
    ...
    this.searchTextApiList = JSON.parse(JSON.stringify(this.postList)).map(
      (item) => {
        delete item.children;
        return item;
      }
  ...
  methods: {
    ...
    onSearch(val) {
      ...
      if (val === "") {
        ...
        this.postList2 = this.searchTextApiList.filter(
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        if (rst.data.isSuccessed) {
          ...
          if (this.postList2 && this.postList2.length === 0) {
            this.$emit(
              "afterDeleteSuccess",
              { departmentId: this.postList[0].parentId, key: selectedRowKeys },
              true
            );
          }
          this.$emit("afterDeleteSuccess", {
            departmentId: this.postList[0].parentId,
            key: selectedRowKeys,
          });
