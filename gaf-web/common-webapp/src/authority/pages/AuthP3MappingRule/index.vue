    ...
    <gaf-table-layout v-show="!open">
      ...
      <template #default>
        <gaf-table-with-page
          ...
          :columns="columns.filter(item => item.dataIndex !== 'mappingRuleId')"
          style="width: 100%;"
          bordered
          size="small"
          align="center"
  ...
  methods: {
    ...
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (this.pagination.current !== 1 && selectedRowKeys.length === this.authP3MappingRuleList.length){
    ...
    async handleDelete(row) {
      ...
      this.$nextTick(() => {
        if (this.pagination.current !== 1 && this.authP3MappingRuleList.length === 1){
          this.pagination.current--
        }
        this.getList()
      })
