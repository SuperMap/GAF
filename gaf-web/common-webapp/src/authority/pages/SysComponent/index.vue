      ...
      <gaf-table-layout>
        ...
        <template #default>
          ...
          <gaf-table-with-page
            ...
            :loading="loading"
            @change="tableChange"
            :row-key="(r) => r.sysComponentId"
            :columns="
              columns.filter((item) => item.dataIndex !== 'sysComponentId')
            "
            class="table-style"
            size="middle"
          >
...
export default {
  ...
  methods: {
    ...
    async batchDel() {
      ...
      if (selectedRowKeys.length !== 0) {
        ...
        this.$nextTick(() => {
          if (
            this.pagination.current !== 1 &&
            selectedRowKeys.length === this.sysComponentList.length
          ) {
    ...
    rowSelect(record, selected, selectedRows) {
      console.log("278", record, selected, selectedRows, selectedRows.length);
    ...
    async handleDelete(row) {
      const url =
        `/authority/${"sys_component".replace("_", "-")}s/` +
        row.sysComponentId;
      ...
      this.$nextTick(() => {
        if (
          this.pagination.current !== 1 &&
          this.sysComponentList.length === 1
        ) {
          this.pagination.current--;
        }
        this.getList();
