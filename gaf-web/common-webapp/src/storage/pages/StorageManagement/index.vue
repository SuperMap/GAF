      ...
      <div class="breadcrumb">
        <a-breadcrumb :routes="routes">
          <template slot="itemRender" slot-scope="{ route, params, routes, paths }">
            ...
            <a href="javascript:;" v-else @click.stop="() => spanClick(route, params, routes, paths)">
              {{ route.breadcrumbName }}
            </a>
          </template>
        </a-breadcrumb>
  ...
  methods: {
    spanClick(route, params, routes) {
      routes.forEach((element,index) => {
        if (element.path === route.path && element.breadcrumbName === route.breadcrumbName) {
    ...
    addbreadcrumb(row) {
      ...
      this.routes.push({path:row.name,breadcrumbName:name[name.length - 2]})
    },
    popRoutes() {
