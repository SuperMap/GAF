 ...
 * @author dqc
 * @date:2021/3/25
 * /analysis/spatial/overlay-iobject
 */
@Path("/")
@Api(value = "空间分析-叠加分析接口-iobject")
@Component
public class OverlayIobjectResource {
    @Autowired
    private OverlayIobjectService overlayIobjectService;


    @ApiOperation(value = "数据集叠加分析(Iobject实现)", notes = "对两个数据集或一个数据集一个Geo对象进行叠加分析(Iobject实现)")
    @ApiImplicitParams({
    ...
    public MessageResult<Recordset> overlayViaIobject(OverlayAnalystParamVO overlayAnalystParamVO){
        Recordset recordset = overlayIobjectService.overlayViaIobject(overlayAnalystParamVO);
...
//        Recordset resultData = JSON.parseObject(data,Recordset.class);
        return MessageResult.successe(Recordset.class).data(recordset).build();
    }
