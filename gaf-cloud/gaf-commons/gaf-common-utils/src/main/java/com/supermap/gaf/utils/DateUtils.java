...
/**
 * 描述:公共日期工具类
 * 此类继承了org.apache.commons.lang3.time.DateUtils
 *
 * @author:yj
 * @date:2021/3/25 父类中的原生方法也可以调用
 */
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static boolean isLeapYEAR(Date date) {
        ...
        if (year % 4 == 0 && (year % 100 != 0 | year % 400 == 0)) { //NOSONAR
            return true;
        } else {
            return false;
        }
     ...
     * @return 天数
     */
    public static long getDayByMinusDate(Object fromDate, Object toDate) throws InvalidAlgorithmParameterException {
        Date f = DateUtils.chgObject(fromDate);
        Date t = DateUtils.chgObject(toDate);
        if (f == null || t == null) {
            throw new InvalidAlgorithmParameterException("传入参数无法解析为正确的时间");
        }
        long fd = f.getTime();
        long td = t.getTime();
