...
GAF平台提供了地图应用基础工程，方便实现GIS应用开发者在此基础上快速程编码和部署自己的大型应用，并可方便把地应用功能整合到系统菜单中。平台使用SuperMap GIS桌面产品iDesktopX和它的GAF扩展插件进行数据管理，并将数据发布成iServer上的数据服务和地图服务，并在应用中使用，开发者可以对GAF提供的基础地图应用中底图、资源目录、工具条等做配置修改，快速产出满足自己的用户需求的地图应用。
